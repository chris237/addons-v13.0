...
class ContributionRegisterEmployee(models.TransientModel):
    _name = 'contribution.register.employee'
    _description = 'Contribution Registers by Employee'

    employee_ids = fields.Many2many(
        'hr.employee', 'emp_reg_rel', 'employee_id', 'wiz_id', string="Employees", required=True)
    date_from = fields.Date(string='Date From', required=True,
                            default=time.strftime('%Y-%m-01'))
    date_to = fields.Date(string='Date To', required=True, default=str(
