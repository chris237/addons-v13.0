...
class payroll_reg(models.TransientModel):
    ...
    employee_ids = fields.Many2many('hr.employee', 'payroll_register_rel',
                                    'payroll_year_id', 'employee_id', string='Employees', required=True)
    rule_ids = fields.Many2many('hr.salary.rule', 'payroll_register_rel_salary',
                                'reg_id', 'rule_id', string='Salary Rules', required=True)
    ...
    def print_report(self):
        ...
        if datas['form'].get('xls_output', False):
            ...
            # actid = self.env['ir.model.data'].get_object_reference('base', 'action_attachment')[1]
            actid = self.env.ref('base.action_attachment')[0]
            myres = actid.read()[0]
            myres['domain'] = "[('id','in',[" + \
                ','.join(map(str, [ir_attachment])) + "])]"
            return myres
