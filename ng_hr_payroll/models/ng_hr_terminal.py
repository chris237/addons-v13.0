...
class notice_policy(models.Model):
    ...
    company_base = fields.Selection(selection=[
        ...
        ('gross', 'Gross Salary'),
        ('fixed', 'Fix Amount'),
        ('', '')], string='Company Payment Basis', index=False, default='fixed')
    emp_base = fields.Selection(selection=[
        ('basic', 'Basic Salary'),
        ('gross', 'Gross Salary'),
        ('fixed', 'Fix Amount'),
        ('', '')], string='Employee Payment Basis', index=False, default='fixed')
    notes = fields.Text(string='Notes')
    company_value = fields.Float(
        string='Company Value', help='If Basis is Fixed Amount, then set value as a fixed amount.')
    emp_value = fields.Float(
        string='Employee Value', help='If Basis is Fixed Amount, then set value as a fixed amount.')
...
class terminal_policy(models.Model):
    ...
    allowance_ids = fields.Many2many(
        'hr.salary.rule', 'terminal_allow_rel', 'terminal_id', 'allow_id', string='Allowances')
    company_id = fields.Many2one(
    ...
    base = fields.Selection(selection=[
        ...
        ('allow', 'Allowances'),
        ('fixed', 'Fix Amount'),
        ('', '')], string='Based On', required=True, index=True, default='fixed')
    ...
    value_month = fields.Float(
        string='Month Value', help='Enter number of months terminal pay due based on years completed with the company.')
    retrospect = fields.Boolean(
        string='Retrospect', help='Tick is you want to count the years from employment date otherwise it will count from qualified date.')
...
class hr_employee_category(models.Model):
    ...
    terminal_policy = fields.Many2one(
        'terminal.policy', string='Terminal Policy')
