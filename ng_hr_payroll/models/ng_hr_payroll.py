...
class working_time(models.Model):
    ...
    shift_id = fields.Many2one(
        'work.shift', string='Shift', default=_get_shift)
    ...
    def check_weekoff(self):
        ...
        for w in self.attendance_ids:
            if w.dayofweek in weekoff:
                raise Warning(
                    _('You can not create working time on week off day'))
        return True
...
class payroll_advice(models.Model):
    ...
    name = fields.Char(string='Name', readonly=True, required=True, states={
        ...
        for advice in self:
            ...
            for slip in slip_ids:
                if not slip.employee_id.bank_account_id or not slip.employee_id.bank_account_id.acc_number:
                    raise Warning(_('Please define bank account for the %s employee') % (
...
class hr_payslip_run(models.Model):
    ...
    mid_month = fields.Boolean(
        string='Mid Month Payroll', help='Tick if you want to process for mid month payroll.')
...
class payroll_advice_line(models.Model):
    ...
    bank_id = fields.Many2one(
        related='bank_account_id.bank_id', relation='res.bank', string='Bank', store=True)
    bank_name = fields.Char(related='bank_id.name',
                            string='Bank Name', store=True)
...
class hr_payslip(models.Model):
    ...
    def onchange_employee_id(self, date_from, date_to, employee_id=False, contract_id=False):
        ...
        old_ot_days_ids = self.ids and ot_obj.search(
            [('payslip_id', '=', self.ids[0])]) or False
        if old_ot_days_ids:
...
#             structure_ids.extend(self.env['hr.payroll.structure']._get_parent_structure(structure_ids))
            structure_ids.extend(
                payslip.struct_id._get_parent_structure().ids)  # probuse
        else:  # probuse
...
#            return super(hr_payslip, self).get_payslip_lines(contract_ids, payslip_id, context)
    ...
    def create(self, vals):
        ...
        if vals.get('company_id', False):
            ...
            if len(pays) > company:
                raise Warning(
                    _('You are running payroll more than the setting on the configuration of company'))
                pass
    ...
    def compute_sheet(self):
        ...
        if context.get('bonus', False):
            ...
            for payslip in self:
                ...
                old_slipline_ids = slip_line_pool.search(
                    [('slip_id', '=', payslip.id)])
        #            old_slipline_ids
                if old_slipline_ids:
                ...
                lines = [(0, 0, line) for line in payslip.get_payslip_lines(
                    ...
                    if context.get('base', 'basic'):
                        if context.get('merge', True):
                            ...
                            if line[2].get('code') == context.get('base'):
                                ...
                                percent_line.update(
                                    {'amount': percent_line['amount'] * context.get('percent', '1.0')})
